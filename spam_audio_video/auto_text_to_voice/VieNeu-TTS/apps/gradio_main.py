...
def main():
    ...
    last_error = None
    for port in range(server_port, server_port + 20):
        try:
            demo.queue().launch(server_name=server_name, server_port=port, share=share)
            return
        except OSError as e:
            if "Cannot find empty port" in str(e):
                print(f"[WARN] Port {port} dang duoc su dung, thu port tiep theo...")
                last_error = e
                continue
            raise
