...
class VideoPipelineConfig:
    ...
    video_preset: str = os.getenv("VIDEO_PRESET", "throughput")
    video_crf: int = 18
    video_cq: int = 18
...
class VideoPipeline:
    ...
    def build_story_renderer_input(
        self,
        project_id: str,
        session_id: str,
        config: VideoPipelineConfig,
        ...
        if not analysis.get("benchmark_override") and current_audio_seconds > 0 and (
        ...
        return {
            ...
            "video": {
                ...
                "duration_seconds": round(
                    audio_seconds
                    if audio_seconds > 0
                    else len(image_files) * per_scene_duration - max(0, len(image_files) - 1) * transition_seconds,
                    3,
                ),
    ...
    def render_video(
        ...
        if not self._check_ffmpeg():
            raise RuntimeError("ffmpeg is required for final MP4/audio mux.")
        ...
        output_ratio = (float(width) / float(height)) if height > 0 else 0.0
        if not (1.70 <= output_ratio <= 1.95 and fps == 60 and selected_encoder == "h264_nvenc"):
            raise RuntimeError(
                "Production video pipeline is locked to the optimized Rust/D3D11/NVENC full-timeline renderer: "
                "16:9 output, 60fps, h264_nvenc. Adjust the web render settings to the production profile."
            )

        renderer_bin = self._resolve_story_renderer_bin()
        ...
        timeline_input_path = dirs["renders_dir"] / f"{Path(safe_name).stem}.render_input.json"
        timeline_report_path = dirs["renders_dir"] / f"{Path(safe_name).stem}.render_report.json"
        mux_timeout = max(300, int(math.ceil(max(audio_seconds, per_scene_duration * total) * 2)))

        if progress_callback:
            progress_callback({
                "stage": "video_render_timeline",
                "current": 0,
                "total": total,
                "message": f"Rendering production timeline with {total} scenes.",
                "files_done": 0,
                "preview_text": safe_name,
            })
        if should_stop and should_stop():
            raise RuntimeError("STOP_REQUESTED")

        timeline_started_at = time.perf_counter()
        timeline_payload = self._render_story_timeline(
            renderer_bin=renderer_bin,
            input_path=timeline_input_path,
        ...
        timeline_elapsed = round(time.perf_counter() - timeline_started_at, 3)
        if progress_callback:
            progress_callback({
                "stage": "video_render_timeline",
                "current": total,
                "total": total,
                "message": "Rendered production timeline.",
                "files_done": total,
        ...
        visual_payload = timeline_payload.get("visual_overlay") if isinstance(timeline_payload, dict) else {}
        visual_overlay = {
            **(visual_payload if isinstance(visual_payload, dict) else {}),
            "logo_path": self._safe_rel((visual_payload or {}).get("logo_path")) if isinstance(visual_payload, dict) and visual_payload.get("logo_path") else "",
            "shader_overlay": True,
            "timeline_renderer": "story_gpu_timeline",
        }

        render_with_audio_path = ""
        session_video_audio_path = ""
        render_timings: dict[str, float] = {
            "timeline_render_s": timeline_elapsed,
            "combine_clips_s": 0.0,
            "visual_overlay_s": 0.0,
        }
        if render_with_audio:
            if should_stop and should_stop():
                raise RuntimeError("STOP_REQUESTED")
            audio_path = self._resolve_audio_for_merge(dirs)
            render_with_audio_path, session_video_audio_path, mux_elapsed, audio_name = self._attach_audio_to_render(
                dirs=dirs,
                silent_path=silent_path,
                safe_name=safe_name,
                audio_path=audio_path,
                mux_timeout=mux_timeout,
            )
            render_timings["mux_audio_s"] = mux_elapsed
            if progress_callback:
                progress_callback({
        ...
        manifest = {
            ...
            "video_preset": str(config.video_preset or "throughput"),
            "video_cq": int(config.video_cq or 18),
            "render_workers": 1,
            "visual_overlay": visual_overlay,
            "story_renderer_bin": self._safe_rel(renderer_bin),
            "timeline_input": self._rel(timeline_input_path),
            "timeline_report": self._rel(timeline_report_path),
            "timings_s": {
                **render_timings,
    ...
    def _attach_audio_to_render(
        ...
        *,
        ...
        elapsed = round(time.perf_counter() - mux_started_at, 3)
        if session_audio_copy != rendered_with_audio:
            shutil.copy2(rendered_with_audio, session_audio_copy)
        return self._rel(rendered_with_audio), self._rel(session_audio_copy), elapsed, audio_name
    ...
    def _resolve_story_renderer_bin(self) -> Path:
        exe_name = "story_gpu_renderer.exe" if os.name == "nt" else "story_gpu_renderer"
        candidates: list[Path] = []
        env_path = str(os.getenv("SPAM_VIDEO_RENDERER_BIN") or "").strip()
        if env_path:
        ...
        candidates.extend([
            self.repo_root / "renderers" / "story_gpu_renderer" / "target" / "release" / exe_name,
        ...
        raise FileNotFoundError(f"Production story renderer binary not found. Checked: {checked}")

    def _render_story_timeline(
        self,
        *,
        renderer_bin: Path,
        input_path: Path,
        ...
        payload = self.build_story_renderer_input(project_id, session_id, config, output_path)
        input_path.parent.mkdir(parents=True, exist_ok=True)
        report_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        input_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        duration_seconds = float((payload.get("video") or {}).get("duration_seconds") or 0.0)
        cmd = [
            str(renderer_bin),
            "render",
            "--config",
        ...
        self._run_cmd(cmd, timeout=max(600, int(max(1.0, duration_seconds) * 12)))
        return payload
